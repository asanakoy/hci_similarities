function [ output_args ] = computeAverageNN(model,category,ridx)
%COMPUTEAVERAGENN Summary of this function goes here
%   Detailed explanation goes here


load(['/export/home/mbautist/Desktop/projects/cnn_similarities/datasets/OlympicSports/labels_HIWIs/labels_',category,'.mat'])

frameID = labels(ridx).anchor;
...
elseif strcmp(model,'ours')
    path_sim = ['/export/home/mbautist/Desktop/projects/cnn_similarities/compute_similarities/sim_matrices/ours/simMatrix_',category,'_',category,'_LR_0.001_M_0.9_BS_128_iter_20000_fc7_prerelu.mat'];
...
load(['/export/home/mbautist/Desktop/projects/cnn_similarities/datasets/OlympicSports/similarities_lda/simMatrix_',category,'.mat'],'image_names');
...
[~, NNs] = sort(simMatrix(frameID, idxs), 'descend');
flipval = flipval(frameID, :);

blending_image = zeros(imsize, imsize, 3, 'double');
w_sum = 0.0;
for i = 1:length(NNs(1:nnns))
    fparts=strsplit(image_names(NNs(i),3:end),'/');
...
anchor_path = fullfile(path_images,image_names(frameID,3:end));
anchor_image = im2double(imresize(imread(anchor_path),[imsize, imsize]));

figure
subplot(1,3,1); imshow(anchor_image); title(['Anchor ', num2str(ridx)]);
